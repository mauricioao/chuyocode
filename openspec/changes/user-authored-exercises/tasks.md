...
- [x] 3.6 GREEN `src/pages/api/auth/confirm.ts` (NEW): `verifyOtp({type:'email', token_hash})` → `setAll` → 303 to `safeNextPath(next)` — **CORRECTED in slice 3b on branch `fix/auth-confirm-pkce-code`.** Testing the link end to end against production showed it signed nobody in: the task specifies only the `token_hash` credential, which is what a CUSTOM email template sends, and custom templates are unavailable on this project's Supabase plan. Every link the site actually sends is the default one and arrives as `?code=`, so the route now also redeems `exchangeCodeForSession(code)`, with `code` taking precedence and no fallback. `code` was also missing from `STRIPPED_PARAMS`, so the T2 defense had a hole. See `apply-progress/slice-03b-confirm-pkce.md`
